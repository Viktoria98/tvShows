...
Plugin.registerCompiler({
  extensions: ['coffee', 'litcoffee', 'coffee.md', 'cjsx']
}, () => new CoffeeCompiler());
...
export class CoffeeCompiler extends CachingCompiler {
  ...
  compileOneFile(inputFile) {
    ...
    try {
      if (inputFile.getExtension() === 'cjsx') {
        coffeeSource = cjsx(source);
      }
      output = coffee.compile(coffeeSource, compileOptions);
    } catch (e) {
      inputFile.error({
        message: e.message,
