...
import LineGraphLegend from './lineGraphLegend.jsx';
...
class LineGraph extends React.Component {
  ...
  componentWillReceiveProps (nextProps) {
    if (nextProps.label !== this.props.label) {
      this.setState({
        selectedMetric: nextProps.savedMetric || [],
      });
    }
  }
  ...
  showTooltip (tooltipData) {
    ...
    return this.setState({
      tooltipData: {
        ...
        data: {
          ...tooltipData.data,
          value: formatNumber(tooltipData.data.value),
        },
      },
    });
  }
  ...
  render () {
    ...
    const dates = _.uniqBy(data.map((d) => d.date.toString()))
      .map((d) => new Date(d));
    ...
    if (isLog) {
      ...
      let maxVal =
        Math.max.apply(null, map) > 0 ? Math.max.apply(null, map) : d3.max(data, (d) => d.value);
    ...
    dataNest.forEach((d, i) => {
      ...
      selected = _.includes(this.state.selectedMetric, d.key);
      if (selected) {
    ...
    allPreparedStatsArray = _.uniqBy(allPreparedStatsArray.map((d) => d.date))
      .map((d) => new Date(d));
          ...
          <svg className="graph" width={this.state.width} height={height}>
            <g transform={`translate(${margins[3]}, ${margins[0] - 5})`}>
              <XAxis
                ...
                h={h}
                translate={{
                  x: allPreparedStatsArray.length === 1 ? 600 : 0,
                  y: h + 10,
                }}
                ...
                isLog={isLog}
              />
              <YAxis y={y} translate="40" align="left" isLog={isLog} />
              <YAxis y={y} translate={w - 5} align="right" isLog={isLog} />
